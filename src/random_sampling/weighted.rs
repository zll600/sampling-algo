...
mod tests {
    ...
    fn test_weighted_sample_errors() {
        ...
        assert_eq!(
            weighted_sample(&data, &weights, 2, true, Some(42)),
            Err(SamplingError::InvalidParameters("All weights must be non-negative".to_string()))
        );
        
        let weights = vec![0.0, 0.0, 0.0]; // All zero weights
        assert_eq!(
            weighted_sample(&data, &weights, 2, true, Some(42)),
            Err(SamplingError::InvalidParameters("At least one weight must be positive".to_string()))
        );

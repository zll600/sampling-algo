    ...
    S: std::hash::Hash + Eq + Clone,
{
    ...
    for (item, stratum) in data.iter().zip(strata.iter()) {
        strata_map.entry(stratum.clone())
...
mod tests {
    ...
    fn test_stratified_sample_proportional() {
        ...
        let sample = stratified_sample(&data, &strata, 6, AllocationMethod::Proportional, Some(42)).unwrap();
    ...
    fn test_stratified_sample_optimal() {
        ...
        let sample = stratified_sample(&data, &strata, 4, AllocationMethod::Optimal(std_devs), Some(42)).unwrap();
    ...
    fn test_stratified_sample_empty() {
        ...
        let sample = stratified_sample(&data, &strata, 0, AllocationMethod::Equal, Some(42)).unwrap();
